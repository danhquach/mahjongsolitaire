...
- **Feedback endpoint** (`/worker`, issue #118): the in-game "Send feedback" form posts to `/api/feedback`, served in production by a Cloudflare Worker script (`worker/index.mjs`, see [decision 0019](docs/decisions/0019-feedback-worker-endpoint.md)); screenshots/recordings ride the same JSON body as base64 (issue #130, [decision 0020](docs/decisions/0020-feedback-attachments-inline-base64.md)). `node --test worker/test/*.test.mjs` for its headless suite. To exercise it locally alongside `npm run dev`, run `npx wrangler dev` from the repo root (no install needed beyond `npx`) — `ui/vite.config.ts` proxies `/api` to its default port (8787).
