...
**Tile sets, pulled forward as earned cosmetics (issue #229, 2026-09-05).** Two purchasable glyph sets ship in a Shop, paid for with Daily trophies rather than IAP (decision 0038) — the themes/tile-set item above stays for whatever is *sold*, not earned. Board felts followed the same day as plain colours in the palette table (decision 0039) — no art needed. Tile backs landed once their sheet was generated and cut (decision 0040), and avatar frames closed the ticket (decision 0041) — the first look another player sees, so the id containment is structural (a table lookup, never the string). Issue #239 then moved the shop's entry point out of Settings into the booster rail's meta group and made the shop a grid of preview-first cards: a currency the player earns on the board needs its shop within reach of the board.
