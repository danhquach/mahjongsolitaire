...
- **QA before commit.** Run the full test suites from a clean install (`core`: `npm ci && npm test`; bench: `node --test bench/test/*.test.mjs`) and verify the ticket's acceptance criteria. All green before any commit.
