...
test('looks keep an unknown id opaquely and drop garbage; the stamp is a non-negative integer or null', () => {
  assert.deepEqual(validateRecord({ looks: { felt: 'lantern', glyphs: 'glyphs-fantasy' }, looksAt: 5 }).looks, {
    felt: 'lantern',
    glyphs: 'glyphs-fantasy',
  });
  assert.equal(validateRecord({ looks: { felt: 'lantern', glyphs: 'glyphs-fantasy' }, looksAt: 5 }).looksAt, 5);
  assert.deepEqual(validateRecord({ looks: { glyphs: 'not an id!' } }).looks, { felt: 'lantern', glyphs: 'lantern' });
  assert.deepEqual(validateRecord({ looks: ['glyphs-fantasy'] }).looks, { felt: 'lantern', glyphs: 'lantern' });
...
test('looks keep a kind this Worker does not know, sorted and bounded (ui/src/profile.ts parseLooks)', () => {
  ...
  assert.ok(Object.keys(validateRecord({ looks: many }).looks).length <= 10);
});

test('owned items merge as a union; the later look stamp wins the whole look', () => {
  const a = { ...EMPTY_RECORD, owned: ['glyphs-fantasy'], looks: { felt: 'lantern', glyphs: 'glyphs-fantasy' }, looksAt: 2000 };
  const b = {
    ...EMPTY_RECORD,
    owned: ['glyphs-calligraphy', 'glyphs-fantasy'],
    looks: { felt: 'lantern', glyphs: 'glyphs-calligraphy' },
    looksAt: 1000,
  };
  ...
  assert.deepEqual(mergeRecords(EMPTY_RECORD, b).looks, { felt: 'lantern', glyphs: 'glyphs-calligraphy' });
...
test('equal look stamps with different content resolve the same way from either side', () => {
  const a = { ...EMPTY_RECORD, looks: { felt: 'lantern', glyphs: 'glyphs-calligraphy' }, looksAt: 1000 };
  const b = { ...EMPTY_RECORD, looks: { felt: 'lantern', glyphs: 'glyphs-fantasy' }, looksAt: 1000 };
  assert.deepEqual(mergeRecords(a, b), mergeRecords(b, a));
  assert.equal(mergeRecords(a, b).looksAt, 1000);
  // Both unstamped but different (a hand-edited record): still commutative.
  const c = { ...EMPTY_RECORD, looks: { felt: 'lantern', glyphs: 'glyphs-fantasy' } };
...
test('cosmetics round-trip through register, sync and read, exercising the 0007 columns', async () => {
  const env = { DB: createDb() };
  const { json: created } = await registerPlayer(env, makeDeps(), {
    record: { ...EMPTY_RECORD, trophies: 30, owned: ['glyphs-calligraphy'], looks: { felt: 'lantern', glyphs: 'glyphs-calligraphy' }, looksAt: 1000 },
  });
  assert.deepEqual(created.profile.record.owned, ['glyphs-calligraphy']);
  assert.deepEqual(created.profile.record.looks, { felt: 'lantern', glyphs: 'glyphs-calligraphy' });
  ...
  assert.deepEqual(JSON.parse(row.looks), { felt: 'lantern', glyphs: 'glyphs-calligraphy' });
  ...
  const response = await handleProfile(
    request('POST', '/api/profile/sync', {
      headers: bearer(created.code),
      body: {
        record: { ...EMPTY_RECORD, trophies: 90, owned: ['glyphs-fantasy'], looks: { felt: 'lantern', glyphs: 'glyphs-fantasy' }, looksAt: 2000 },
      },
    }),
    env,
    makeDeps(),
  ...
  assert.deepEqual(profile.record.looks, { felt: 'lantern', glyphs: 'glyphs-fantasy' });
  ...
  assert.deepEqual(stored.looks, { felt: 'lantern', glyphs: 'glyphs-fantasy' });
