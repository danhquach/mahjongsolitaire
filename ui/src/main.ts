...
import { mountLeaderboardPanel } from './leaderboard-panel.js';
...
async function start(): Promise<void> {
  ...
  // The end-of-level celebrations (issues #120 / #121 / #122): the win's
  // cascade and lanterns, the loss's slam beat, the deadlock's grey-out, and
  // the delayed dialog each one ends in. Not a panel — the dialog is the
  // level's result, outside the stack — so nothing registers here.
  const celebrations = mountCelebrations({
    reducedMotion: () => settings.value.reducedMotion || prefersReducedMotion(),
    board: {
      score: () => game.score,
      presentTiles: () => game.board.presentTiles(),
      nearPairs: () => nearPairs(game.board),
    },
    feedback,
    animator,
    holder,
  ...
  function showStatus(opts: { readonly fromResume?: boolean } = {}): void {
    ...
    if (status === 'won') {
      ...
      leaderboard.submitRun(game.score, elapsed.ms);
    ...
    const wayOut = canShuffle
      ...
      : canUndo
        ...
        : overlayRestart.hidden
          ...
          : overlayRestart;
    if (status === 'won') {
      celebrations.presentWin(wayOut);
    } else if (status === 'lost') {
      celebrations.presentLoss(wayOut, opts.fromResume ?? false);
    } else {
      celebrations.presentStuck(wayOut, opts.fromResume ?? false);
    }
  }
  ...
  /** The weekly board (issues #70, #176) lives in leaderboard-panel.ts
   *  (issue #244): its panel, its countdown, the opt-in row in the profile and
   *  the post that follows a clear. This call is its place in the stack — the
   *  one panel that opens over the end-of-level dialog — and the whole of what
   *  it may reach. */
  const leaderboard = mountLeaderboardPanel({
    panels,
    storage,
    announcer,
    settingsButton,
    ...
    deal: () => ({
      ...
      shuffles: shuffleCount,
      moves: game.snapshot().stack.moves as unknown as Record<string, unknown>[],
    }),
  });
  ...
  /** The first-run tutorial's card and its spotlight (issues #59, #150) live
   *  in tutorial-panel.ts (issue #244). First in the stack so Escape reaches
   *  it first. This is the whole of what it may reach: the board's tiles and
   *  their rects, Hint's highlight for step 3, a redraw, and focus back to
   *  the board when it comes down. */
  const tut = mountTutorialPanel({
    panels,
    announcer,
    board: {
      spotTiles: () => {
      ...
      label,
      describePair,
      peekHint: () => game.peekHint(),
    },
    setHighlight: (pair) => {
      hintPair = pair;
    },
    redraw,
    focusBoard: () => a11y.focusActive(),
  ...
  /** The Cloud sync section of the profile (issue #138) lives in
   *  cloud-sync.ts (issue #244). This is the whole of what it may reach, and
   *  the three things it has to tell the rest of the game. */
  const cloud = mountCloudSync({
    storage,
    announcer,
    profile,
    record,
    // The server's record was merged in: another device may have bought or
    // picked a look (issue #229), and the balance may have moved, so a Buy
    // waiting for its Confirm is withdrawn.
    onRecordAdopted: () => {
    ...
    // The leaderboard opt-in is gated on sync being on.
    onCredentialsChanged: () => leaderboard.renderSection(),
  });
  ...
  function confirmCopy(action: ConfirmAction): { title: string; text: string; button: string } {
    ...
    const synced = cloud.credentials !== null;
    if (action === 'reset') {
      return {
  ...
  async function runConfirmedAction(): Promise<void> {
    ...
    if (cloud.credentials !== null) {
      const result =
        confirmAction === 'reset' ? await resetAccount(cloud.credentials) : await closeAccount(cloud.credentials);
      ...
      if (!result.ok && !alreadyGone) {
        ...
        confirmStatus.textContent =
          result.reason === 'unauthorized'
            ...
            : cloud.failureText(result.reason);
        confirmBusy = false;
        renderConfirmControls();
  ...
  function useBooster(kind: BoosterKind): void {
    ...
    if (result.ok && (kind === 'undo' || kind === 'shuffle')) {
      trayFx.clear();
      // celebrations.cancel() already calls animator.clear(); a direct call
      // here too would be redundant. The still-stuck branch skips
      // celebrations.cancel() (it would drop the wash), so it clears the
      // animator itself.
    ...
    if (stillStuck) celebrations.restoreStuckWash();
  ...
  function finishTap(
    ...
    if (outcome.kind === 'matched') {
      ...
      // 'fail' cue and everything after it is celebrations.presentLoss's job,
  ...
  function startTutorialIfArmed(): void {
    if (settings.value.showTutorial) tut.start();
  }
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'hidden') {
      ...
      animator.clear();
      trayFx.clear();
      // Not celebrations.cancel(): a pending end-of-level dialog (win or
  ...
  if (resumed === null && settings.value.showTutorial) tut.start();
  ...
  (window as unknown as Record<string, unknown>)['__slice'] = {
    ...
    animating(): boolean {
      return animator.busy || trayFx.busy || celebrations.busy;
    },
