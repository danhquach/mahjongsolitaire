...
import type { SyncFailure } from './sync.js';
...
async function start(): Promise<void> {
  ...
  const cloud = mountCloudSync({
    ...
    onCredentialsChanged: () => renderBoardSection(),
  });
  ...
  async function loadBoard(announce: boolean): Promise<void> {
    ...
    const result = await fetchWeeklyBoard(cloud.credentials);
  ...
  function submitRunResult(score: number, elapsedMs: number): void {
    if (cloud.credentials === null || !boardOptIn) return;
    void submitRunScore(cloud.credentials, {
      score,
  ...
  boardOptInInput.addEventListener('change', () => {
    ...
    void withdrawFromBoard(cloud.credentials).then((result) => {
      setBoardStatus(
        result.ok
